...
func init() {
	android.RegisterModuleType("yuki_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = yukiExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = yukiExpandVariables(ctx, dep_file)
	...
	cmd := yukiExpandVariables(ctx, String(g.properties.Cmd))
